...
export function endDataTrace(trace: DataTrace | undefined, durationMs = trace ? Date.now() - trace.startedAt : 0): DataTraceSummary | undefined {
  ...
  return { app: trace.app, action: trace.action, path: trace.path, traceId: trace.traceId, requestId: trace.requestId, firestoreReturnedDocuments: trace.totals.FIRESTORE, estimatedFirestoreBillableReads, clientCacheRecords: trace.totals.CLIENT_CACHE, appCacheRecords: trace.totals.APP_CACHE, memoryRecords: trace.totals.MEMORY, staticRecords: trace.totals.STATIC, unknownRecords: trace.totals.UNKNOWN, totalReturnedRecords: totalDocuments, firestoreDocuments: trace.totals.FIRESTORE, clientCacheDocuments: trace.totals.CLIENT_CACHE, appCacheDocuments: trace.totals.APP_CACHE, memoryDocuments: trace.totals.MEMORY, staticDocuments: trace.totals.STATIC, unknownDocuments: trace.totals.UNKNOWN, totalDocuments, operations: trace.operations, durationMs, level: classifyDataTraceLevel(Math.max(totalDocuments, estimatedFirestoreBillableReads)), records: trace.records.map(record => ({ ...record, durationMs: record.durationMs })), };
}
