...
export function startDataTrace(input: DataTraceInput): DataTrace | undefined {
  ...
  return { ...input, action: input.action || "request", path: input.path || "unknown", traceId: input.traceId || randomTraceId(), startedAt: Date.now(), records: [], aggregateKeys: new Set(), totals: { FIRESTORE: 0, SNAPSHOT: 0, CLIENT_CACHE: 0, APP_CACHE: 0, MEMORY: 0, STATIC: 0, UNKNOWN: 0 }, operations: 0 };
}
...
export function recordDataAccess(trace: DataTrace | undefined, input: DataAccessInput) {
  ...
  trace.operations += 1;
  if (trace.records.length < MAX_RECORDS) trace.records.push({ ...input, source, app: input.app || trace.app, action: input.action || trace.action, path: input.path || trace.path, traceId: trace.traceId, ...(trace.requestId ? { requestId: trace.requestId } : {}), documents: returnedDocuments, returnedDocuments, firestoreReads, estimatedBillableReads, level: classifyDataTraceLevel(Math.max(returnedDocuments, estimatedBillableReads)) });
}
...
export function endDataTrace(trace: DataTrace | undefined, durationMs = trace ? Date.now() - trace.startedAt : 0): DataTraceSummary | undefined {
  ...
  const estimatedFirestoreBillableReads = trace.records.reduce((sum, record) => sum + (record.source === "FIRESTORE" ? record.estimatedBillableReads : 0), 0);
  return { app: trace.app, action: trace.action, path: trace.path, traceId: trace.traceId, requestId: trace.requestId, firestoreReturnedDocuments: trace.totals.FIRESTORE, estimatedFirestoreBillableReads, snapshotRecords: trace.totals.SNAPSHOT, clientCacheRecords: trace.totals.CLIENT_CACHE, appCacheRecords: trace.totals.APP_CACHE, memoryRecords: trace.totals.MEMORY, staticRecords: trace.totals.STATIC, unknownRecords: trace.totals.UNKNOWN, totalReturnedRecords: totalDocuments, firestoreDocuments: trace.totals.FIRESTORE, clientCacheDocuments: trace.totals.CLIENT_CACHE, appCacheDocuments: trace.totals.APP_CACHE, memoryDocuments: trace.totals.MEMORY, staticDocuments: trace.totals.STATIC, unknownDocuments: trace.totals.UNKNOWN, totalDocuments, operations: trace.operations, durationMs, level: classifyDataTraceLevel(Math.max(totalDocuments, estimatedFirestoreBillableReads)), records: trace.records.map(record => ({ ...record, durationMs: record.durationMs })), };
}
