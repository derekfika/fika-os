...
function card(app) {
  ...
  return `<article class="card"><div class="card-top"><h3>${app.name}</h3><span class="status ${status}"><i></i>${status}</span></div>${app.planned ? '<span class="planned">Coming soon</span>' : ""}<p>${app.description}</p><div class="url">${app.url.replace("http://", "")}</div><button class="open" data-open="${app.id}">Open app <span>↗</span></button></article>`;
}
