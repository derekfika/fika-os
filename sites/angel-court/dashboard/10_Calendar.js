function createCalendarEventForRow(rowNumber, options) {
  ...
  const warnForLateAmendment = shouldWarnForStaleCalendarRegeneration_(
    isStaleRegeneration,
    start,
    new Date()
  );
...
function makeCalendarTitle_(booking) {
  const locationShortCode =
    getConfiguredValue_("LOCATION_SHORT_CODE", CONFIG.LOCATION_SHORT_CODE || "OAC");
