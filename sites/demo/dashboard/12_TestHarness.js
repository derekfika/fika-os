...
function testCalendarHelpers_() {
  ...
  const attendeeConfig = parseCalendarAttendees_("derek@fikacatering.com, manager@example.com, operations@example.com");
  const selectedAttendees = selectCalendarAttendees_(attendeeConfig, {
    attendeeEmails: ["manager@example.com"]
  });
  assertDashboardEqual_(selectedAttendees.length, 1, "Selected calendar attendees should use only ticked emails.");
  assertDashboardEqual_(selectedAttendees[0].email, "manager@example.com", "Selected calendar attendee email failed.");
...
function testDemoDeliveryCharge_() {
  const booking = makeDashboardTestBooking_();
  booking.totalPrice = 120;
  booking.items = [
  ...
  assertDashboardTest_(
    withoutDelivery.items.every(function(item) { return item.itemId !== "demo_cpu_delivery_charge"; }),
