// =============================================================================
// PARSER — 58VE booking XLSX → booking object
// =============================================================================
...
//   • "20:30" etc.                      (valid 24-hour time retained)
...
/**
 * Parses any time representation found in 58VE booking forms and
 ...
 *  - Valid 24-hour values such as "20:30"
 *
...
function parseHospitalityTime_(raw) {
  ...
  // Time ranges always represent a service window. Use the start, never the
  // end: "6pm-10pm" -> "18:00", "18:30 – 22:00" -> "18:30".
  const rangeStart = extractStartTimeFromRange_(t);
  ...
  if (m) {
    ...
    if (!isValidHospitalityMinute_(mm)) return "";
    return pad2_(hh) + ":" + mm;
  }
...
/**
 * Retains valid 24-hour values. The helper name is preserved because it is
 * used by the shared dashboard parser.
 * @param  {number} hh
 * @return {number}
 */
function collapseHospitalityHour_(hh) {
  const hour = Number(hh);
  if (!isFinite(hour) || hour < 0) return 0;
  return hour % 24;
}

function isValidHospitalityMinute_(mm) {
  const minute = Number(mm);
  return isFinite(minute) && minute >= 0 && minute <= 59;
}
 ...
 * Kept so existing call sites (parse58VeBookingSheet_, GmailScanner,
...
/**
 * Parses any date representation found in 58VE booking forms and
...
function extractStartTimeFromRange_(text) {
  ...
  const match = value.match(
    new RegExp("\\b(" + token + ")\\s*(?:-|\\bto\\b)\\s*(" + token + ")\\b", "i")
  );

  if (!match) return "";

  let start = String(match[1] || "").trim();
  const end = String(match[2] || "").trim();

  if (!/(am|pm)$/i.test(start)) {
    const endMeridian = end.match(/(am|pm)$/i);

    if (endMeridian) {
      const startHour = Number((start.match(/^\d{1,2}/) || [""])[0]);
      const endHour = Number((end.match(/^\d{1,2}/) || [""])[0]);

      // "6-10pm" means 6pm. "11-1pm" conventionally crosses noon,
      // so the starting 11 is AM.
      const inferred =
        endMeridian[1].toLowerCase() === "pm" &&
        startHour > endHour
          ? "am"
          : endMeridian[1].toLowerCase();

      start += inferred;
    }
  }

  return parseHospitalityTime_(start);
}
...
function extractDateFromText_(text) {
  ...
  if (m) {
    const date = parseComponents_(m[1], m[2], m[3]);
    return date ? formatIsoDate_(date) : "";
  }

  m = value.match(/\b(\d{1,2})(?:st|nd|rd|th)?\s+([a-z]+)\s+(\d{2,4})\b/i);
  return m ? parseHospitalityDate_(m[0]) : "";
}
 ...
 * because it is referenced in parse58VeLineItems_.
 */
...
function buildBookingFromMessageId(messageId) {
  ...
  try {
    ...
    if (!candidate) throw new Error("NOT_BOOKING_FORM: XLSX does not match the 58VE hospitality form.");
...
function parse58VeBookingSheet_(sheet, context) {
  context = context || {};
  ...
  function findValueNearLabel(labelText) {
    const direct =
      findValueRightOfLabel(labelText) ||
      findDisplayRightOfLabel(labelText);
    ...
    for (let r = 0; r < values.length; r++) {
      for (let c = 0; c < values[r].length; c++) {
        if (norm(values[r][c]) !== target && norm(displayValues[r][c]) !== target) {
          continue;
        }

        for (let rowOffset = 1; rowOffset <= 2; rowOffset++) {
          for (let colOffset = 0; colOffset <= 4; colOffset++) {
            const raw =
              values[r + rowOffset] &&
              values[r + rowOffset][c + colOffset];
            const display =
              displayValues[r + rowOffset] &&
              displayValues[r + rowOffset][c + colOffset];
            const candidate =
              raw !== "" && raw !== null && raw !== undefined ? raw : display;

            if (
              candidate !== "" &&
              candidate !== null &&
              candidate !== undefined &&
              !isFormLabel_(candidate)
            ) {
              return candidate;
            }
          }
  ...
  const roomRaw =
    findValueNearLabel("Meeting Room number / Location:") ||
    findValueNearLabel("Meeting Room number / Location") ||
    findValueNearLabel("Room / Location:") ||
    findValueNearLabel("Floor Level") ||
    findValueNearLabel("Floor level:");
  ...
  const items = parse58VeLineItems_(sheet);
  ...
  if (!eventDate) {
    eventDate =
      extractDateFromText_(context.attachmentName) ||
      extractDateFromText_(context.subject) ||
      extractDateFromText_(sheet.getName()) ||
      "";
  }
  ...
  booking.location = String(
    getConfiguredValue_(
      "LOCATION_NAME",
      CONFIG.DEFAULT_LOCATION || "58 Victoria Embankment"
    )
  ).trim();
  booking.floor = String(roomRaw || "").trim();
  booking.notes = notes;
...
function parse58VeLineItems_(sheet) {
  const values = sheet.getDataRange().getValues();
  const displayValues = sheet.getDataRange().getDisplayValues();
  ...
  for (let r = headerRow + 1; r < values.length; r++) {
    ...
    const timeText =
      ...
      extractTimeFromText_(commentText) ||
    ...
    items.push({
      section: currentSection || infer58VeSectionFromRow_(r + 1),
      time: timeText,
      name: split.name,
      detail: split.detail,
      info: infoText,
...
/**
 * The 58VE template separates menu groups with blank rows instead of named
 * section rows. These bands match the supplied form.
 */
function infer58VeSectionFromRow_(sheetRow) {
  const row = Number(sheetRow || 0);

  if (row >= 17 && row <= 25) return "Refreshments";
  if (row >= 27 && row <= 32) return "Breakfast";
  if (row >= 34 && row <= 41) return "Lunch";
  if (row >= 43 && row <= 48) return "Events Catering";
  if (row >= 50 && row <= 52) return "Drinks";
  if (row >= 53 && row <= 56) return "Operational Extras";
...
function is58VeBookingForm_(sheet) {
  return classify58VeBookingForm_(sheet).isBookingForm;
}

function classify58VeBookingForm_(sheet) {
  const values = sheet.getDataRange().getDisplayValues();
  ...
  let hasQuantityHeader = false;
  let hasTotalHeader = false;
  for (let r = 0; r < values.length; r++) {
    const row = values[r].map(norm);
    if (row.includes("item")) hasItemHeader = true;
    if (row.some(v =>
      v.includes("number required") ||
      v === "quantity" ||
      v === "qty" ||
      ...
      hasQuantityHeader = true;
    }
    if (row.some(v => v.includes("total cost"))) hasTotalHeader = true;
    if (hasItemHeader && hasQuantityHeader) {
      hasItemHeader = true;
    }
  }

  const titleMatch = values
    .slice(0, 12)
    .some(row => row.some(cell => {
      const text = norm(cell);
      return text.includes("hospitality booking form") ||
        text.includes("tenants booking form");
    }));

  const siteInstruction = values
    .slice(0, 20)
    .some(row => row.some(cell => norm(cell).includes("58ve@fikacatering.com")));
  ...
  const locationLabel =
    hasText("Meeting Room number / Location:") ||
    hasText("Meeting Room number / Location") ||
    hasText("Floor Level") ||
    hasText("Room / Location:");

  const grandTotal = hasText("Grand Net Total") || hasText("GRAND NET TOTAL");
  const coreLabelCount = coreLabels.filter(Boolean).length;

  let score = 0;
  if (titleMatch) score += 4;
  if (siteInstruction) score += 2;
  if (hasItemHeader && hasQuantityHeader) score += 4;
  if (hasTotalHeader) score += 1;
  if (grandTotal) score += 3;
  if (locationLabel) score += 1;
  score += coreLabelCount;
...
function find58VeBookingSheet_(spreadsheet) {
  const candidates = spreadsheet.getSheets()
    .map(sheet => ({
      sheet,
      classification: classify58VeBookingForm_(sheet)
    }))
    .sort((a, b) => b.classification.score - a.classification.score);

  if (!candidates.length || !candidates[0].classification.isBookingForm) {
    return null;
  }
...
function hasMeaningful58VeBookingData_(booking) {
