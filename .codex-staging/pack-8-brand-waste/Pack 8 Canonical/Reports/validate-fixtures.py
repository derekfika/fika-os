...
def validate(schema, rule, value, path="$"):
    ...
    if expected_type == "object":
        ...
        if "exclusiveMinimum" in rule and value <= rule["exclusiveMinimum"]:
            errors.append(f"{path}: must be greater than exclusive minimum")
...
for schema_path in sorted((root / "Schemas").glob("*.schema.json")):
    ...
    for expected, folder in (("pass", "valid"), ("fail", "invalid")):
        fixture_path = root / "Schemas" / "fixtures" / folder / f"{folder}-{stem}.json"
...
(root / "Reports" / "schema-validation-report.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
