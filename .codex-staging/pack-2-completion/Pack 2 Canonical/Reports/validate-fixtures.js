...
for (const schemaFile of schemas) {
  ...
  for (const [kind, shouldPass] of [['valid', true], ['invalid', false]]) {
    ...
    const data = readJson(path.join(root, 'Schemas', 'fixtures', kind, fixture));
...
fs.writeFileSync(path.join(root, 'Reports', 'schema-validation-report.json'), `${JSON.stringify(output, null, 2)}\n`, 'utf8');
