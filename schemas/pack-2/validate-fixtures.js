...
let failures = 0;
const results = [];
const schemas = fs.readdirSync(root).filter(f => f.endsWith('.schema.json')).sort();
...
fs.writeFileSync(path.join(root, 'schema-validation-report.json'), `${JSON.stringify(output, null, 2)}\n`, 'utf8');
