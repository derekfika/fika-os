import { CANONICAL_ALLERGEN_COLUMNS } from "./allergen-contract";
...
const signature = (label: string, value: { printedName: string; signatureDataUrl?: string; signedAt: string }) => `<div class="signature"><strong>${label}</strong>${value.signatureDataUrl ? `<img src="${escapeHtml(value.signatureDataUrl)}" alt="${escapeHtml(label)} signature">` : ""}<span>${escapeHtml(value.printedName)}</span><small>${escapeHtml(new Date(value.signedAt).toLocaleString("en-GB"))}</small></div>`;

/** The single signed publication matrix renderer used by Menu Planning, Drive archive and CPU print view. */
