...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === "clean-duplicate-entries") {
      ...
      return NextResponse.json({ snapshot: result.snapshot, removed: result.removed, weeks: listWeeks(), blockers: validateWeek(result.snapshot), publicationState: publicationState(result.snapshot) });
    }
    if (action === "publish") {
