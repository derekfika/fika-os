...
import { CANONICAL_ALLERGEN_COLUMNS, normaliseOperationalAllergens, resolveNoKeyAllergenState, toggleOperationalAllergen, type CanonicalAllergenMap, type CanonicalAllergenKey } from "@/lib/fika-contracts";

function displayAllergenState(allergens: CanonicalAllergenMap, key: string): string {
  if (key === "no_key_allergens") return resolveNoKeyAllergenState(allergens);
...
export default function AllergenChecker() {
  ...
  const editDraft = (entryId: string, entry: typeof entries[number], next: Partial<AllergenDraft>) => { const current = draftFor(entryId, entry); setDrafts(value => ({ ...value, [entryId]: { ...current, ...next, allergens: normaliseOperationalAllergens(next.allergens || current.allergens) } })); setSavedMessage(""); };
