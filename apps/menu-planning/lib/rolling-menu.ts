...
export async function batchUpdateEntries(weekId: string, expectedWeekVersion: number, updates: Array<{ entryId: string; allocations: RollingAllocation[] }>, actor = "local-menu-planner") {
  ...
  return withMenuPlanningTransaction(state => {
    ...
    for (const update of updates) {
      if (seen.has(update.entryId)) throw Object.assign(new Error("A portion row was submitted more than once."), { status: 422 });
      seen.add(update.entryId);
      const entry = snapshot.entries.find(candidate => candidate.id === update.entryId);
      if (!entry) throw Object.assign(new Error("A portion row was not found. Refresh before saving."), { status: 404 });
      ...
      entry.audit.push({ action: "portion-allocations-batch-saved", at: now(), by: actor });
    }
