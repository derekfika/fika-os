...
type LiveGovernedOploc = { canonicalId: string; label: string };
const normaliseDestination = (allocation: RollingAllocation, liveOplocs: readonly LiveGovernedOploc[] = []): RollingAllocation => {
  const liveById = new Map(liveOplocs.map(oploc => [oploc.canonicalId, oploc]));
