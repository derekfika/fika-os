...
import { notifyCpuPublicationChanged } from "./publication-notification";
...
export function createPublishedMenuDay(weekId: string, dayId: string, signoff: MenuPublicationSignoff, actor = "local-menu-planner") {
  ...
  const hadWithdrawal = publication.days.some(value => value.sourceDayId === day.sourceDayId && value.status === "withdrawn");
  ...
  void notifyCpuPublicationChanged({ event: "publication_changed", publicationDayId: publishedDay.publicationDayId, serviceDate: publishedDay.date, version: publishedDay.version, action: hadWithdrawal ? "republished" : version === 1 ? "published" : "amended" });
...
export function withdrawPublishedMenuDay(publicationId: string, publicationDayId: string, reason: string, actor = "local-menu-planner") {
  ...
  const at = now(); day.status = "withdrawn"; day.withdrawal = { actor, at, reason: reason.trim() }; publication.audit.push({ action: "menu-day-withdrawn", at, by: actor, publicationDayId }); write(stored);
  ...
  void notifyCpuPublicationChanged({ event: "publication_changed", publicationDayId: day.publicationDayId, serviceDate: day.date, version: day.version, action: "withdrawn" });
  return clone(publication);
}
