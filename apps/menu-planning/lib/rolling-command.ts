...
export function applyRollingEntryPatch(entry: RollingEntry, patch: RollingEntryPatch) {
  ...
  Object.assign(entry, { ...patch, ...(patch.itemLabel !== undefined ? { itemLabel: String(patch.itemLabel).trim() } : {}), ...(dishChanged && !restoringReview ? { allergens: {}, mayContainNotes: "", allergenReviewInvalidated: true } : {}) });
  return dishChanged;
}
