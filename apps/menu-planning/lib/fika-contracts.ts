...
export function enforceNoKeyExclusivity(input: CanonicalAllergenMap): CanonicalAllergenMap { const result = { ...input }; const namedKeys = CANONICAL_ALLERGEN_KEYS.filter(key => key !== "no_key_allergens"); const hasPositiveNamed = namedKeys.some(key => result[key] === "contains" || result[key] === "may_contain"); const allNamedExplicitClear = namedKeys.every(key => result[key] === "clear"); result.no_key_allergens = hasPositiveNamed ? "clear" : allNamedExplicitClear ? "contains" : "unrecorded"; return result; }
export function deriveNoKeyAllergens(input: CanonicalAllergenMap): CanonicalAllergenMap { const result = { ...input }; const namedKeys = CANONICAL_ALLERGEN_KEYS.filter(key => key !== "no_key_allergens"); const hasPositiveNamed = namedKeys.some(key => result[key] === "contains" || result[key] === "may_contain"); result.no_key_allergens = hasPositiveNamed ? "clear" : namedKeys.every(key => result[key] === "clear") ? "contains" : "unrecorded"; return result; }
export function normaliseOperationalAllergens(input: Record<string, unknown> | undefined): CanonicalAllergenMap { const result: CanonicalAllergenMap = {}; for (const [rawKey, rawValue] of Object.entries(input || {})) { const key = toCanonicalAllergenKey(rawKey); if (key && (rawValue === "clear" || rawValue === "contains" || rawValue === "may_contain" || rawValue === "unrecorded")) result[key] = rawValue; } return enforceNoKeyExclusivity(result); }
export function toggleOperationalAllergen(current: CanonicalAllergenMap, key: CanonicalAllergenKey): CanonicalAllergenMap { const state = current[key] || "clear"; const next = state === "clear" ? "contains" : state === "contains" ? "may_contain" : "clear"; if (key === "no_key_allergens") return enforceNoKeyExclusivity({ ...current, ...Object.fromEntries(CANONICAL_ALLERGEN_KEYS.filter(candidate => candidate !== key).map(candidate => [candidate, "clear"])), [key]: next }); return enforceNoKeyExclusivity({ ...current, [key]: next }); }
