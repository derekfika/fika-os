...
export async function readDeliveredInOplocs(request: NextRequest): Promise<GovernedOploc[]> {
  ...
  const arrangementBody = await arrangementResponse.json() as { arrangements?: Array<{ oplocId: string; serviceLabel?: string; lifecycleState?: string; effectiveFrom?: string; effectiveTo?: string }>; error?: { message?: string } };
  ...
  return oplocBody.oplocs.filter(item => eligible.has(item.canonicalId));
}
