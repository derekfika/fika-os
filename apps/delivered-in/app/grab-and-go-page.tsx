...
export default function GrabAndGoPage() {
  ...
  const siteHref = (path: string, view: "today" | "week" | "allergens" = "today") => deliveredInHref({ view, oplocId: siteId || undefined, week: currentLocation.week, day: currentLocation.day }, path);
  const site = sites.find(item => item.oplocId === siteId);
  return <main className="app-shell"><header className="compact-header"><div className="brand-lockup"><strong>FIKA OS</strong><span className="brand-divider" /> <span className="leaf-mark">♧</span><b>Delivered-In</b></div><div className="header-actions">{site && <label className="site-picker"><span>Site</span><select value={siteId} onChange={event => { setSiteId(event.target.value); window.history.replaceState(null, "", siteHref("/grab-and-go")); }}>{sites.map(item => <option key={item.oplocId} value={item.oplocId}>{item.label}</option>)}</select></label>}<span className="avatar">DB</span></div></header><nav className="top-nav" aria-label="Delivered-In navigation"><a className="nav-link" href={siteHref("/")}>▣ <span>Today</span></a><a className="nav-link" href={siteHref("/", "week")}>▦ <span>This week</span></a><a className="nav-link" href={siteHref("/", "allergens")}>♧ <span>Allergens</span></a><a className="nav-link active" aria-current="page" href={siteHref("/grab-and-go")}>▤ <span>Grab &amp; Go</span></a></nav>{error ? <section className="empty-state"><p className="error" role="alert">{error}</p></section> : site ? <GrabAndGoView oplocId={site.oplocId} /> : <section className="content"><div className="empty-state"><p>Loading authorised sites…</p></div></section>}</main>;
}
