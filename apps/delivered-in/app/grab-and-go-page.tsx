...
export default function GrabAndGoPage() {
  ...
  useEffect(() => { void fetch("/api/delivered-in/access?service=grab-and-go", { cache: "no-store" }).then(async response => { const body = await response.json(); if (!response.ok) throw new Error("Unavailable"); setSites(body.sites || []); setSiteId(body.sites?.[0]?.oplocId || ""); }).catch(() => setError("This ordering service is temporarily unavailable. Please check back shortly.")); }, []);
