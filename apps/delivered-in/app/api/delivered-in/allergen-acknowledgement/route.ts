...
export async function POST(request: NextRequest) {
  ...
  try { const resolved = await resolveAccess(request); assertAuthorisedOploc(resolved.access, body.siteId); const state = await acknowledgeAllergenSafety(body.siteId, body.serviceDate!, body.releaseVersion, resolved.access.email, new Date().toISOString()); return NextResponse.json({ state }); } catch (error) { return NextResponse.json(deliveredInErrorBody(error, "Acknowledgement could not be saved."), { status: Number((error as { status?: number }).status) || 502 }); }
}
