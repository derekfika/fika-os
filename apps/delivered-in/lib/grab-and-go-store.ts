...
const read = (): Stored => { if (!existsSync(file)) return { version: 1, orders: [] }; try { const value = JSON.parse(readFileSync(file, "utf8")) as Partial<Stored>; return { version: 1, orders: value.orders || [] }; } catch { return { version: 1, orders: [] }; } };
...
export function readGrabAndGoCatalogue(): GrabAndGoProduct[] { try { const value = JSON.parse(readFileSync(catalogueFile, "utf8")) as { products?: GrabAndGoProduct[] }; return value.products || []; } catch { return []; } }
