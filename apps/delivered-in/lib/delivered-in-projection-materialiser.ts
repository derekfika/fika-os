...
type Review = { entries: Map<string, { allergens: Record<string, "clear" | "contains" | "may_contain">; mayContainNotes?: string }>; cpuReview: { status: "pending" | "signed"; signatures: Array<{ role: string; printedName: string; signedAt: string }>; drivePdfUrl?: string }; orderIds: string[]; updatedAt?: string };
...
export async function buildDeliveredInDayProjection(input: { request: NextRequest; site: Site; day: ProjectedDay; loadReview: ReviewLoader; governed: boolean }): Promise<DeliveredInDayProjection> {
  ...
  const projection: DeliveredInDayProjection = {
    ...
    sourceLineage: {
      ...
      cpu: { orderIds: review?.orderIds || [], ...(review?.updatedAt ? { updatedAt: review.updatedAt } : {}) },
      deliveredIn: { ...(artifact?.artifactId ? { siteMenuArtifactId: artifact.artifactId } : {}), generatedAt: new Date().toISOString() },
    },
    generatedAt: new Date().toISOString(),
    state: {
      freshness: "current",
      completeness: review ? "complete" : "partial",
      menu: input.day.entries.length ? "present" : "empty",
      cpu: review ? review.cpuReview.status === "signed" ? "present" : "pending" : "unavailable",
      exceptions: [
