...
export async function forwardProductionMaterialisation(input: ExternalProductionMaterialisation, options: { allowPending?: boolean } = {}) {
  ...
  if (result.cpuHandoff === "pending" && !options.allowPending) throw new Error("Integration Hub materialised the Production Order, but CPU projection handoff is pending.");
  return result.cpuHandoff || "delivered";
}
