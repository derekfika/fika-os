...
export async function applyCpuReleaseEvent(request: NextRequest, event: CpuReleaseEvent) {
  ...
  const receipt = await completeCpuReleaseReceipt(identity, { result: "applied", projectionId: projection.projectionId, projectionContentHash: projection.sourceLineage.menu.contentHash, artifactId: artifact.artifactId, driveFileId: artifact.driveFileId });
