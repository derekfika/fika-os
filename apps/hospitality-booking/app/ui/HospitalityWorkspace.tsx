...
export default function HospitalityWorkspace() {
  ...
  return <BookingPortal key={active.id} siteKey={siteKey(active)} oplocId={active.id} siteLabel={active.label} availableSites={sites} onSiteChange={next => { const selected = sites.find(site => site.id === next); if (!selected) return; setActiveId(selected.id); window.localStorage.setItem(rememberedKey, selected.id); window.history.pushState({}, "", `/workspace?oploc=${encodeURIComponent(selected.id)}`); }} />;
}
