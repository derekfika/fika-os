...
export async function POST(request: NextRequest) {
  try {
    const body = await request.json() as { name?: string; html?: string; pdfBase64?: string; weekCommencing?: string };
    ...
    try {
      const saved = await saveGoogleDrivePdf({ name: body.name.trim(), pdfBase64: body.pdfBase64, owner: { type: "app-workspace", appId: "cpu-production" }, folderId: process.env.GOOGLE_DRIVE_CPU_PRODUCTION_FOLDER_ID, weekCommencing: body.weekCommencing, folderLabel: "CPU production" });
