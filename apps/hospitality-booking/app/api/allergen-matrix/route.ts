...
export async function GET(request: NextRequest) {
  ...
  try {
    const candidates = [...new Set([bookingId, `production-order:v1:${bookingId}`, `production-order:${bookingId}`])];
    for (const candidate of candidates) {
      const response = await fetch(`${cpuBase()}/api/production-plan?orderId=${encodeURIComponent(candidate)}`, { cache: "no-store" });
      const body = await response.json() as { plan?: { matrixArtifact?: unknown }; error?: { message?: string } };
