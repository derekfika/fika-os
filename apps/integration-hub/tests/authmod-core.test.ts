...
test("delegation is bounded, linked to a live source, and becomes ineffective with the source", async () => {
  ...
  const sourceGrant = await grantAuthority(repository, { subjectId: source.id, subjectType: "interactive", actor: admin, appId: "cpu-production", resource: "production.allergen-final-approve", action: "Approve", scope: { kind: "organisation", ids: [] }, reason: "Source authority." });
  const result = await createDelegation(repository, { delegatorId: source.id, delegateId: delegate.id, sourceGrantId: sourceGrant.id, action: "Approve", scope: { kind: "organisation", ids: [] }, effectiveFrom: "2026-09-01T00:00:00.000Z", effectiveTo: "2026-09-10T00:00:00.000Z", actor: admin, reason: "Holiday cover." });
  ...
  await assert.rejects(() => createDelegation(repository, { delegatorId: source.id, delegateId: delegate.id, sourceGrantId: result.grant.id, action: "Approve", scope: { kind: "organisation", ids: [] }, effectiveFrom: "2026-09-01T00:00:00.000Z", effectiveTo: "2026-09-10T00:00:00.000Z", actor: admin, reason: "Recursive." }), /not currently effective/);
});
...
test("special authority is explicit and action separation is preserved", async () => {
  ...
  assert.equal((await evaluateAuthority(repository, { principal, appId: "menu-planning", resource: "menu.publish", action: "Publish", scope: { kind: "oploc", ids: ["oploc:mnk"] } })).allowed, false);
  await assert.rejects(() => grantAuthority(repository, { subjectId: person.id, subjectType: "interactive", actor: admin, appId: "menu-planning", resource: "menu.publish", action: "Publish", scope: { kind: "oploc", ids: ["oploc:mnk"] }, reason: "Invalid site-scoped publisher." }), /organisation-wide/);
  await grantAuthority(repository, { subjectId: person.id, subjectType: "interactive", actor: admin, appId: "menu-planning", resource: "menu.publish", action: "Publish", scope: { kind: "organisation", ids: [] }, reason: "Approved publisher." });
...
test("Workspace rows can be committed one at a time without duplicating identities", async () => {
  const repository = makeRepo(); const operator = await identity(repository, "row-import-admin@example.test"); const actor: AuthPrincipal = { type: "interactive", id: operator.id, displayName: operator.displayName }; await grantAuthmodAdmin(repository, { identityId: operator.id, actor: admin, reason: "Row import administrator." });
  const workbook = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet([{ "First Name [Required]": "One", "Last Name [Required]": "Person", "Email Address [Required]": "one@example.test", "Status [READ ONLY]": "Active" }, { "First Name [Required]": "Two", "Last Name [Required]": "Operations", "Email Address [Required]": "two@example.test", "Status [READ ONLY]": "Active" }]), "Users");
  const preview = await previewAccessImport(repository, { buffer: Buffer.from(XLSX.write(workbook, { type: "buffer", bookType: "xlsx" })), filename: "workspace-rows.xlsx", actor }); const first = preview.resolutions[0]; const second = preview.resolutions[1];
  const firstResult = await commitAccessImport(repository, { importId: preview.record.id, actor, decisions: { [first.id]: { accept: true, identityKind: "person", createIdentity: { displayName: "One Person", email: "one@example.test" } } }, idempotencyKey: `row:${first.id}` }); assert.equal(firstResult.status, "partial"); assert.equal((await repository.listIdentities()).filter(value => value.provenance === "import").length, 1);
  const secondResult = await commitAccessImport(repository, { importId: preview.record.id, actor, decisions: { [second.id]: { accept: true, identityKind: "person", createIdentity: { displayName: "Two Operations", email: "two@example.test" } } }, idempotencyKey: `row:${second.id}` }); assert.equal(secondResult.status, "committed"); assert.equal((await repository.listIdentities()).filter(value => value.provenance === "import").length, 2);
  const retry = await commitAccessImport(repository, { importId: preview.record.id, actor, decisions: {}, idempotencyKey: `row:${second.id}` }); assert.equal(retry.committedRows, 2); assert.equal((await repository.listIdentities()).filter(value => value.provenance === "import").length, 2);
});
...
test("operational authority fails closed when OPLOC scope is omitted", async () => {
  ...
  await grantAuthority(repository, { subjectId: person.id, subjectType: "interactive", actor: admin, appId: "menu-planning", resource: "menu.publish", action: "Publish", scope: { kind: "organisation", ids: [] }, reason: "Organisation publisher." });
...
test("operational identities receive normal scoped access but not person-required authority or Full Access", async () => {
  ...
  const sign = await grantAuthority(repository, { subjectId: operational.id, subjectType: "interactive", actor: admin, appId: "cpu-production", resource: "production.allergen-sign", action: "Approve", scope: { kind: "organisation", ids: [] }, reason: "Explicit operational allergen signing authority." });
  const final = await grantAuthority(repository, { subjectId: operational.id, subjectType: "interactive", actor: admin, appId: "cpu-production", resource: "production.allergen-final-approve", action: "Approve", scope: { kind: "organisation", ids: [] }, reason: "Explicit operational final allergen authority." });
...
test("person-required policy denies erroneous operational grants and allows person grants", async () => {
  ...
  const signGrant = await grantAuthority(repository, { subjectId: person.id, subjectType: "interactive", actor: admin, appId: "cpu-production", resource: "production.allergen-sign", action: "Approve", scope: { kind: "organisation", ids: [] }, reason: "Person allergen signer." });
  const finalGrant = await grantAuthority(repository, { subjectId: person.id, subjectType: "interactive", actor: admin, appId: "cpu-production", resource: "production.allergen-final-approve", action: "Approve", scope: { kind: "organisation", ids: [] }, reason: "Person final approver." });
...
test("operational actor audit remains operational and does not claim the custodian acted", async () => {
  ...
  await grantAuthority(repository, { subjectId: operational.id, subjectType: "interactive", actor, appId: "cpu-production", resource: "production.allergen-sign", action: "Approve", scope: { kind: "organisation", ids: [] }, reason: "Operational shared-device signing capability." });
