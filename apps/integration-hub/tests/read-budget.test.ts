...
test("interactive hot paths do not contain whole canonical collection gets", () => {
  ...
  const authmodPackage = readFileSync(new URL("../lib/authmod-reference-read-package.ts", import.meta.url), "utf8");
  assert.match(authmodPackage, /where\("entityType", "==", "Legend"\)/);
  assert.match(authmodPackage, /where\("entityType", "==", "Employment"\)/);
