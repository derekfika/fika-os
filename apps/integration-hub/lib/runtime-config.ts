...
export function getFikaRuntimeConfig(env: RuntimeEnv = process.env) {
  ...
  if (mode === "local") {
    ...
    const useCloudAuth = env.FIKA_LOCAL_GOOGLE_AUTH === "true";
    if (useCloudAuth) {
...
export function hasFirebaseClientConfig(env: RuntimeEnv = process.env) {
  return Boolean(env.NEXT_PUBLIC_FIREBASE_API_KEY && env.NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN && env.NEXT_PUBLIC_FIREBASE_PROJECT_ID && env.NEXT_PUBLIC_FIREBASE_APP_ID);
}
