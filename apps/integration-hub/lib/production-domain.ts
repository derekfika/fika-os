...
export async function createProductionFromApprovedBooking(actor: Actor, bookingId: string, idempotencyKey: string, conversions: Record<string, { quantity: number; unit: string }> = {}) {
  ...
  return db.runTransaction(async transaction => {
    ...
    stageDomainEvent(transaction, createDomainEvent({ eventType: "production.order.created", sourceAggregateId: order.canonicalId, sourceVersion: order.version, occurredAt: now, correlationId: idempotencyKey, payload: { canonicalId: order.canonicalId, version: order.version, status: order.status, sourceBookingId: order.sourceBookingId, serviceDate: order.serviceDate, productionLocationId: order.productionLocationId, destinationLabel: order.destinationLabel, lineIds: order.lines.map(line => line.canonicalId) } }));
...
export async function transitionProductionOrder(actor: Actor, canonicalId: string, expectedVersion: number, status: ProductionStatus, reason: string) {
  ...
  return db.runTransaction(async transaction => {
    ...
    stageDomainEvent(transaction, createDomainEvent({ eventType: status === "cancelled" ? "production.order.withdrawn" : "production.order.amended", sourceAggregateId: next.canonicalId, sourceVersion: next.version, occurredAt: now, payload: { canonicalId: next.canonicalId, version: next.version, status: next.status, reason, sourceBookingId: next.sourceBookingId, serviceDate: next.serviceDate, productionLocationId: next.productionLocationId, destinationLabel: next.destinationLabel, lineIds: next.lines.map(line => line.canonicalId) } }));
    return next;
  });
}
...
export async function updateProductionLines(actor: Actor, canonicalId: string, expectedVersion: number, lines: Array<{ canonicalId: string; productionQuantity?: number; productionUnit?: string; actualQuantity?: number; shortfallQuantity?: number; substitution?: string; wasteQuantity?: number; productionInstructions?: string; dietaries: Record<string, unknown>; allergenEvidenceStatus?: ProductionLine["allergenEvidenceStatus"] }>) {
  return db.runTransaction(async transaction => {
    ...
    transaction.set(ref, next);
    stageDomainEvent(transaction, createDomainEvent({ eventType: "production.order.amended", sourceAggregateId: next.canonicalId, sourceVersion: next.version, occurredAt: now, payload: { canonicalId: next.canonicalId, version: next.version, status: next.status, sourceBookingId: next.sourceBookingId, serviceDate: next.serviceDate, productionLocationId: next.productionLocationId, destinationLabel: next.destinationLabel, lineIds: next.lines.map(line => line.canonicalId) } }));
    return next;
  });
}
