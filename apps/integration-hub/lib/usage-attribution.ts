...
export type AttributionOperation = { operation: string; app: string; route: string; dataset: string; source: string; executions: number; returnedDocuments: number; estimatedBillableReads: number; estimatedWrites: number; estimatedDeletes: number; averageReadsPerExecution: number; maxReadsPerExecution: number; cacheHits: number; cacheMisses: number; fallbackCount: number; packageVersions: number[]; projectionVersions: number[] };
