...
function unavailableAttribution(message: string, range: UsageRange, resolution: UsageResolution, authoritativeReads: number | null): UsageAttribution {
  ...
  return { available: false, message, traceCount: 0, estimatedFirestoreBillableReads: 0, firestoreReturnedDocuments: 0, authoritativeReads, unattributedReads: authoritativeReads === null ? null : authoritativeReads, coveragePercent: authoritativeReads === null || authoritativeReads === 0 ? 0 : 0, overAttribution: false, parseFailures: 0, truncated: false, instrumentedApps: Object.keys(EXPECTED_STAGING_INSTRUMENTATION), appsSeenInWindow: [], expectedInstrumentation: { ...EXPECTED_STAGING_INSTRUMENTATION }, apps: [], actions: [], operations: [], traceDetails: [], traceDetailsTotal: 0, traceDetailsTruncated: false, buckets: Array.from({ length: bucketCount }, (_, index) => ({ timestamp: new Date(Date.parse(range.start) + index * periodMs).toISOString(), cloudMonitoringReads: 0, attributedEstimatedReads: 0, unattributedReads: 0, byApp: {} })), cache: { HIT: 0, MISS: 0, IN_FLIGHT_JOIN: 0, FALLBACK: 0, BYPASS: 0, STALE: 0, REVALIDATED: 0 }, reconciliation: reconcileUsage({ reads: authoritativeReads, writes: null, deletes: null }, { reads: 0, writes: 0, deletes: 0 }, true) };
}
