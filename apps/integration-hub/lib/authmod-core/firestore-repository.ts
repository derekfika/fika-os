...
export class FirestoreAuthModRepository implements AuthModRepository {
  ...
  async listApplications() { return cachedAuthmodReference({ scope: this.cacheScope, name: "listApplications", documents: value => value.length, load: () => readAll<ApplicationRegistryEntry>(collections.applications) }); }
