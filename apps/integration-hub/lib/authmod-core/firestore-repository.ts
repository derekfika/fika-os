...
import { canonicalDocumentId } from "../canonical-oplocs";
...
import { bootstrapAuthmodAccessPackage, bumpAuthmodAccessHead, getAuthmodAccessReadPackage } from "../authmod-access-read-package";
...
async function save<T extends { version: number }>(name: string, id: string, value: T, expectedVersion?: number) {
  await db.runTransaction(async transaction => { const ref = db.collection(name).doc(id); const snapshot = await transaction.get(ref); assertExpectedVersion(snapshot.exists ? Number(snapshot.data()?.version) : undefined, expectedVersion); transaction.set(ref, value as unknown as DocumentData); if (name === collections.applications) bumpCacheDatasets(transaction, ["applications"]); if (name === collections.identities) bumpAuthmodAccessHead(transaction, id); if (name === collections.apps || name === collections.sites || name === collections.grants || name === collections.delegations || name === collections.custodians) bumpAuthmodAccessHead(transaction, String((value as unknown as Record<string, unknown>).identityId || (value as unknown as Record<string, unknown>).subjectId || (value as unknown as Record<string, unknown>).delegateId || (value as unknown as Record<string, unknown>).operationalIdentityId || id)); });
  if ([collections.identities, collections.apps, collections.sites, collections.grants, collections.delegations, collections.custodians].includes(name as typeof collections.identities)) await bootstrapAuthmodAccessPackage(String((value as unknown as Record<string, unknown>).identityId || (value as unknown as Record<string, unknown>).subjectId || (value as unknown as Record<string, unknown>).delegateId || (value as unknown as Record<string, unknown>).operationalIdentityId || id));
...
export class FirestoreAuthModRepository implements AuthModRepository {
  ...
  async listLegendReferences(search = "", limit = 100) { const references = (await this.referencePackage()).legends; const needle = search.trim().toLowerCase(); return references.filter(value => !needle || value.label.toLowerCase().includes(needle)).slice(0, Math.min(limit, 1000)); }
  ...
  async getAuthorityGrant(id: string) { const snapshot = await firestoreRead("delegation.source-grant", collections.grants, () => db.collection(collections.grants).doc(id).get()); return snapshot.exists ? snapshot.data() as AuthorityGrant : undefined; }
  async listAuthorityGrantsForDecision(input: { subjectId: string; subjectType: "interactive" | "service"; appId: string; resource: string; action: AuthorityGrant["action"] }) { const snapshot = await firestoreRead("service.decision-grants", collections.grants, () => db.collection(collections.grants).where("subjectId", "==", input.subjectId).where("subjectType", "==", input.subjectType).where("appId", "==", input.appId).where("resource", "==", input.resource).where("action", "==", input.action).get()); return snapshot.docs.map(document => document.data() as AuthorityGrant); }
  ...
  async saveDelegationWithGrant(input: { delegation: DelegationRecord; grant: AuthorityGrant; audit: AccessAuditEvent }) { await db.runTransaction(async transaction => { transaction.create(db.collection(collections.delegations).doc(input.delegation.id), input.delegation as unknown as DocumentData); transaction.create(db.collection(collections.grants).doc(input.grant.id), input.grant as unknown as DocumentData); bumpAuthmodAccessHead(transaction, input.delegation.delegateId); bumpAuthmodAccessHead(transaction, input.delegation.delegatorId); transaction.create(db.collection(collections.audits).doc(input.audit.id), input.audit as unknown as DocumentData); }); await bootstrapAuthmodAccessPackage(input.delegation.delegateId); await bootstrapAuthmodAccessPackage(input.delegation.delegatorId); invalidateAuthmodReferenceCaches(); invalidateAuthmodAdmissionCache(); }
  async saveStandardApplicationBundle(input: { assignment: AppAssignment; grants: AuthorityGrant[]; audit?: AccessAuditEvent; expectedAssignmentVersion?: number }) {
    await db.runTransaction(async transaction => {
      ...
      bumpAuthmodAccessHead(transaction, input.assignment.identityId);
    ...
    await bootstrapAuthmodAccessPackage(input.assignment.identityId);
    invalidateAuthmodReferenceCaches();
    invalidateAuthmodAdmissionCache();
  }
