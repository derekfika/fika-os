...
export class FirestoreAuthModRepository implements AuthModRepository {
  ...
  async listLegendReferences(search = "", limit = 100) { const [legendSnapshot, employmentSnapshot] = await Promise.all([firestoreRead("listLegendReferences", "integrationHubCanonical", () => db.collection("integrationHubCanonical").where("entityType", "==", "Legend").get()), firestoreRead("listLegendReferences", "integrationHubCanonical", () => db.collection("integrationHubCanonical").where("entityType", "==", "Employment").get())]); const legends = legendSnapshot.docs.map(document => document.data() as CanonicalRecord); const employments = employmentSnapshot.docs.map(document => document.data() as CanonicalRecord); recordAuthmodReadBudget("listLegendReferences", { legends: legendSnapshot.size, employments: employmentSnapshot.size }); const needle = search.trim().toLowerCase(); return legends.filter(value => value.canonicalId && value.lifecycleStatus !== "archived" && !isTerminatedLegend(value, employments) && (!needle || String(value.record.displayName || value.record.preferredName || value.canonicalId).toLowerCase().includes(needle))).slice(0, Math.min(limit, 1000)).map(value => ({ id: value.canonicalId, label: String(value.record.displayName || value.record.preferredName || value.canonicalId), active: true })); }
  ...
  async listCustodianAssignments(operationalIdentityId: string) { const snapshot = await db.collection(collections.custodians).where("operationalIdentityId", "==", operationalIdentityId).get(); return snapshot.docs.map(document => document.data() as CustodianAssignment); }
  ...
  async getActiveOploc(oplocId: string) { const snapshot = await db.collection("integrationHubCanonical").doc(canonicalDocumentId(oplocId)).get(); const record = snapshot.exists ? snapshot.data() as CanonicalOploc : undefined; return record && activeOploc(record, oplocId) ? { id: oplocId, label: String(record.record?.approvedName || oplocId), active: true } : undefined; }
  ...
  async listAuthorityGrants(subjectId: string, subjectType?: "interactive" | "service") { const query = subjectType ? db.collection(collections.grants).where("subjectId", "==", subjectId).where("subjectType", "==", subjectType) : db.collection(collections.grants).where("subjectId", "==", subjectId); const snapshot = await query.get(); return snapshot.docs.map(document => document.data() as AuthorityGrant); }
  ...
  async getDelegation(id: string) { const snapshot = await db.collection(collections.delegations).doc(id).get(); return snapshot.exists ? snapshot.data() as DelegationRecord : undefined; }
  async listDelegations(delegateId?: string) { const query = delegateId ? db.collection(collections.delegations).where("delegateId", "==", delegateId) : db.collection(collections.delegations); const snapshot = await query.get(); return snapshot.docs.map(document => document.data() as DelegationRecord); }
  ...
  async listImports(limit = 100) { const snapshot = await db.collection(collections.imports).orderBy("uploadedAt", "desc").limit(Math.min(limit, 200)).get(); return snapshot.docs.map(document => document.data() as ImportRecord); }
