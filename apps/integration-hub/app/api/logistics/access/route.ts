...
async function handleGet(request: NextRequest) {
  try {
    ...
    const access = await cachedAuthmodAdmission({ identityId: principal.id, appId: "logistics", authorityAction: "app-access", representedOplocId: principal.representedOplocId, primaryCustodianLegendId: session.primaryCustodianLegendId, load: () => resolveUserAccess(new FirestoreAuthModRepository(), { principal, appId: "logistics" }) });
