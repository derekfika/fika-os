...
const Command = z.object({ action: z.literal("notify-booking-confirmed"), sourceBookingId: z.string().min(8) }).strict();
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(await notifyBookingConfirmedForProductionOrder(command.sourceBookingId), { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
