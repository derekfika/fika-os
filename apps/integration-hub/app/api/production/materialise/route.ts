...
export async function POST(request: NextRequest) { try { let actor; if (internalProductionRequestAllowed(request)) actor = { uid: "integration-materialiser", name: "Integration Materialiser", role: "integration-admin", synthetic: true } as const; else { actor = await requireActor(request, ["integration-admin", "reviewer"]); assertPermission(actor, "canonical.edit"); } const input = parseExternalProductionMaterialisation(await request.json()); const result = await materialiseExternalProductionOrder(actor, input); let logisticsHandoff: "delivered" | "pending" = "delivered"; try { const delivery = await deliverLogisticsProjectionForProductionOrder(result.order); if (delivery && delivery.delivery.status !== "delivered") logisticsHandoff = "pending"; } catch { logisticsHandoff = "pending"; } let cpuHandoff: "delivered" | "pending" = "delivered"; try { await notifyCpuProjection(result.order, input.status === "withdrawn" || input.status === "cancelled" ? "withdrawn" : input.status === "amended" ? "amended" : "created", `cpu-projection:${input.sourceDomain}:${input.sourceEntityId}:${input.destinationOplocId}:v${input.sourceVersion}`); } catch { cpuHandoff = "pending"; } return NextResponse.json({ ...result, cpuHandoff, logisticsHandoff }); } catch (error) { return errorResponse(error); } }
