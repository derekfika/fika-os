...
export async function GET(request: NextRequest) { try { const actor = await requireActor(request); assertPermission(actor, "canonical.view"); return NextResponse.json(await serviceDefinitionCatalogueOverview(), noStore()); } catch (error) { return errorResponse(error); } }
