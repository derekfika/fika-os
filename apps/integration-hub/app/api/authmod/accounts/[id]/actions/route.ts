...
const common = { expectedVersion: z.number().int().positive().optional(), reason: z.string().trim().min(3).max(500) };
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    else if (command.action === "site") command.enabled ? await assignSite(context.repository, { identityId: id, oplocId: command.oplocId, actor: context.principal, accessType: command.accessType, reason: command.reason, effectivePeriod: { effectiveFrom: command.effectiveFrom, effectiveTo: command.effectiveTo } }) : await revokeSite(context.repository, { identityId: id, oplocId: command.oplocId, actor: context.principal, reason: command.reason });
    else if (command.action === "app") command.enabled ? await grantStandardApplicationAccess(context.repository, { identityId: id, appId: command.appId, actor: context.principal, accessType: command.accessType, reason: command.reason, effectivePeriod: { effectiveFrom: command.effectiveFrom, effectiveTo: command.effectiveTo } }) : await revokeStandardApplicationAccess(context.repository, { identityId: id, appId: command.appId, actor: context.principal, reason: command.reason });
    ...
    else if (command.action === "authority-grant") await grantAuthority(context.repository, { subjectId: id, subjectType: "interactive", actor: context.principal, appId: command.appId, resource: command.resource, action: command.authorityAction, scope: command.scope, effectivePeriod: { effectiveFrom: command.effectiveFrom, effectiveTo: command.effectiveTo }, reason: command.reason });
