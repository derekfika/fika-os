...
export async function GET(request: NextRequest) { try { const context = await requireAuthmodAdminContext(request); return NextResponse.json({ services: await context.repository.listServicePrincipals() }, { headers: { "Cache-Control": "no-store" } }); } catch (error) { return errorResponse(error); } }
