...
export async function loadAuthmodOptions(): Promise<Response> {
  ...
  if (cached && manifest?.packageVersion === cached.packageVersion) {
    recordDataAccess({ app: "integration-hub", operation: "authmod.options.indexeddb", source: "CLIENT_CACHE", documents: cached.applications.length + cached.oplocs.length + cached.legends.length, cacheHit: true, cacheResult: "HIT", dataset: "authmod-references", packageVersion: cached.packageVersion });
