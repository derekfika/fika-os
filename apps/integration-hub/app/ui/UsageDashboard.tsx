...
export default function UsageDashboard() {
  ...
  return <main className={styles.page}><header className={styles.header}><div><p className={styles.eyebrow}>Integration Hub · Internal operations</p><h1>Usage <em>observatory.</em></h1><p>Server-side visibility into Firestore operation volume after a deploy.</p></div><div className={styles.actions}><button onClick={() => void load(true)} disabled={busy}>Refresh</button><a href="/">Back to FIKA OS</a></div></header>
    ...
    {data && <><section className={styles.cards}>{(["reads", "writes", "deletes"] as UsageMetric[]).map(metric => <article key={metric}><strong>{format.format(data.totals[metric])}</strong><span>{labels[metric]} in range</span><small>Google Cloud Monitoring</small></article>)}<article className={styles.allowance}><strong>{data.allowance ? (data.allowance.percent * 100).toFixed(1) + "%" : "—"}</strong><span>Allowance used</span><small>{data.allowance ? format.format(data.allowance.remaining) + " reads remaining" : "No configured allowance"}</small></article></section>
    <section className={styles.grid}><article className={styles.panel}><div className={styles.heading}><div><p className={styles.eyebrow}>Selected range · {data.resolution} buckets</p><h2>Firestore operations</h2></div></div><div className={styles.timeline}>{(["reads", "writes", "deletes"] as UsageMetric[]).map(metric => <Bars key={metric} label={labels[metric]} points={data.timeline[metric]} />)}</div></article><aside className={styles.panel}><p className={styles.eyebrow}>Read health</p><h2>{data.allowance?.status || "Not configured"}</h2><dl className={styles.facts}><div><dt>Headroom</dt><dd>{data.allowance ? format.format(data.allowance.remaining) : "Not enough configuration"}</dd></div><div><dt>Regression signal</dt><dd>{data.baseline.message}</dd></div><div><dt>Business timezone</dt><dd>Europe/London</dd></div></dl></aside></section>
...
function Bars({ label, points }: { label: string; points: UsagePoint[] }) { const max = Math.max(1, ...points.map(point => point.value)); return <div className={styles.series}><strong>{label}</strong><div className={styles.bars} aria-label={label + " over selected range"}>{points.map(point => <span key={point.timestamp} title={format.format(point.value) + " operations"} style={{ height: Math.max(2, point.value / max * 100) + "%" }} />)}</div></div>; }
