...
function HospitalityBrochureImport({ update, setProgress, reportError }: { update: (state: HubState) => void; setProgress: (text: string) => void; reportError: (text: string) => void }) {
  ...
  const [filter, setFilter] = useState("needs-review"); const [publishing, setPublishing] = useState(false); const [confirmation, setConfirmation] = useState<string | null>(null); const [selected, setSelected] = useState<any | null>(null); const [draft, setDraft] = useState<Record<string, string>>({ offeringMode: "standard", effectiveFrom: new Date().toISOString().slice(0, 10), vatRate: "0.2" });
  ...
  return <Panel title="Hospitality Brochure Review">
    ...
    {confirmation && <ConfirmationModal title="Publish reviewed MNK menu?" description="Every reviewed, schema-valid catalogue record in this import will become available to the portal." confirmLabel="Publish reviewed menu" busy={publishing} onCancel={() => setConfirmation(null)} onConfirm={() => publishImport(confirmation)} />}
