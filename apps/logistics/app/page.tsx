...
function ScrollableRealTimeline({ runs, onStop, onSchedule, onQueueDrop }: { runs: PlannerDay["runs"]; serviceDate: string; onStop: (runId: string, stopId: string) => void; onRun?: (runId: string) => void; onSchedule: (sourceRunId: string, stopId: string, targetRunId: string, time: string, end?: string, lane?: "delivery" | "collection") => void; onQueueDrop: (kind: "group" | "movement", runId: string, targetRunId: string, time?: string, lane?: "delivery" | "collection", collectionRequired?: boolean) => void; }) {
  ...
  const move = (lane: "delivery" | "collection", amount: number) => {
  ...
  const group = (lane: "delivery" | "collection", start: number) => <section className={`stable-group ${lane}-group`}><header className="stable-section-heading"><strong>{lane === "delivery" ? "DELIVERIES" : "COLLECTIONS"} · {timeLabel(start)}–{timeLabel(start + 6)}</strong><span className="timeline-scroll-controls"><button aria-label={`Scroll ${lane} earlier`} disabled={start === 0} onClick={() => move(lane, -1)}>←</button><button aria-label={`Scroll ${lane} later`} disabled={start === 18} onClick={() => move(lane, 1)}>→</button></span></header><div className="stable-viewport"><div className="stable-canvas" style={{ transform: `translateX(-${(start / 24) * 100}%)` }}><div className={`stable-ruler ${lane}-ruler`}><span aria-hidden="true" />{Array.from({ length: 25 }, (_, hour) => <b key={hour} style={{ "--ruler-position": hour } as CSSProperties}>{timeLabel(hour)}</b>)}</div>{runs.map((run, index) => <div className="stable-vehicle-row" key={`${lane}-${run.runId}`}><div className="stable-driver"><strong>{run.vehicle || `Van ${index + 1}`}</strong><span>{run.driver || "Select driver"}</span><small>{run.scheduledStopCount} scheduled · {run.needsTimeStopCount} needs time</small></div><StableTimelineLane run={run} lane={lane} startHour={start} zoom={zoom} onStop={onStop} onSchedule={onSchedule} onQueueDrop={onQueueDrop} /></div>)}</div></div></section>;
