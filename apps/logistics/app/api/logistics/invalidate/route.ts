...
async function handlePost(request: NextRequest) {
  ...
  if (!body.serviceDate || !/^\d{4}-\d{2}-\d{2}$/.test(body.serviceDate) || !body.sourceDomain || !body.sourceEntityId || typeof body.sourceVersion !== "number" || !Number.isInteger(body.sourceVersion) || body.sourceVersion < 1 || !body.changedAt || !body.changeType || !LOGISTICS_PROJECTION_CHANGE_TYPES.includes(body.changeType as typeof LOGISTICS_PROJECTION_CHANGE_TYPES[number])) return NextResponse.json({ error: { message: "A complete Logistics projection invalidation is required." } }, { status: 422 });
