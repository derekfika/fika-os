...
export default function Mobile() {
  ...
  const [issueType, setIssueType] = useState("Unable to complete");
  ...
  const deliveries = stops.filter((stop) => stop.movementType !== "collection" && stop.linkedOperation !== "collection");
  const collections = stops.filter((stop) => stop.movementType === "collection" || stop.linkedOperation === "collection");
  ...
  async function execute(action: string, stop: DeliveryStop, extra: Record<string, unknown> = {}) {
    ...
    setSelectedStop(undefined); setIssueStop(undefined); setIssueText(""); await load();
  }
  async function dispatchRun(run: DeliveryRun) {
    ...
    if (!response.ok) { setError(body?.error || "The deliveries could not be marked as dispatched."); return; }
    await load();
  }
  const reportIssue = () => { if (issueStop && issueText.trim()) void execute("report-issue", issueStop, { issueDescription: `${issueType}: ${issueText.trim()}` }); };
  ...
  return <main className="driver-app">
    ...
    {error && <div className="driver-alert" role="alert">{error}<button onClick={() => void load()}>Retry</button></div>}
    {!data ? <section className="driver-empty">Loading your day…</section> : view === "messages" ? <Messages messages={messages} onDismiss={(id) => setMessages((current) => current.filter((message) => message.id !== id))} onClear={() => setMessages([])} /> : view === "more" ? <More driver={driver} runs={runs} /> : <>
      {runs.filter((run) => run.status === "planned" || run.status === "ready").map((run) => <section className="driver-departure" key={run.canonicalId}><div><p className="driver-section-kicker">LOAD CHECK</p><strong>{run.vehicleLabel || "Your vehicle"} is ready</strong><span>Confirm the deliveries are loaded before leaving.</span></div><button onClick={() => void dispatchRun(run)}>Mark deliveries dispatched</button></section>)}
      <section className="driver-summary" aria-label="Daily summary"><Metric icon="↘" value={deliveries.length} label="Deliveries" tone="purple" /><Metric icon="✓" value={completed} label="Completed" tone="mint" /><Metric icon="↗" value={collections.length} label="Collections" tone="blue" /><Metric icon="!" value={attention} label="Attention" tone="rose" /></section>
      <div className="driver-progress"><span>Today’s progress</span><strong>{completed} of {stops.length} tasks complete</strong><div><i style={{ width: `${stops.length ? (completed / stops.length) * 100 : 0}%` }} /></div></div>
      <nav className="task-switcher" aria-label="Task type"><button className={view === "deliveries" ? "active" : ""} onClick={() => setView("deliveries")}>Deliveries <b>{deliveries.filter((stop) => stop.status !== "completed").length}</b></button><button className={view === "collections" ? "active" : ""} onClick={() => setView("collections")}>Collections <b>{collections.filter((stop) => stop.status !== "completed").length}</b></button></nav>
      <section className="driver-list"><div className="list-heading"><div><p className="driver-section-kicker">{view === "collections" ? "RETURN LOAD" : "DELIVERY ROUTE"}</p><h2>{view === "collections" ? "Collection stops" : "Your stops"}</h2></div><span>{visibleStops.length} total</span></div>{nextStop && <div className="next-banner"><span>Next up</span><strong>{stopLabel(nextStop, data)}</strong><b>{timeFor(nextStop)}</b></div>}{visibleStops.length ? visibleStops.map((stop, index) => <StopRow key={stop.canonicalId} stop={stop} data={data} index={index} onOpen={() => setSelectedStop(stop)} />) : <div className="driver-empty compact">No {view} assigned for this day.</div>}</section>
    </>}
...
function StopRow({ stop, data, index, onOpen }: { stop: DeliveryStop; data: Data; index: number; onOpen: () => void }) { const issue = stop.status === "issue" || (stop.issues || []).some((item) => item.status === "open"); return <button className={`stop-row-card ${stop.status} ${issue ? "attention" : ""}`} onClick={onOpen}><span className="stop-number">{stop.status === "completed" ? "✓" : index + 1}</span><span className="stop-row-main"><span className="stop-row-meta"><b>{stop.status === "completed" ? "COMPLETED" : stop.status === "arrived" ? "CURRENT" : issue ? "ATTENTION" : index === 0 ? "UP NEXT" : "PLANNED"}</b>{stop.notes && <em>✎</em>}</span><strong>{stopLabel(stop, data)}</strong><small>{stop.movementType === "collection" ? "Collection" : "Delivery"} · {timeFor(stop)}{itemSummary(stop) ? ` · ${itemSummary(stop)}` : ""}</small></span><span className="stop-row-arrow">›</span></button>; }
function StopDetail({ stop, data, onClose, onAction, onIssue }: { stop: DeliveryStop; data: Data; onClose: () => void; onAction: (action: string, stop: DeliveryStop) => void; onIssue: () => void }) { const movement = movementsForStop(stop, data.movements)[0]; const oploc = data.oplocs.find((item) => item.id === stop.locationOplocId); const address = oploc?.address || oploc?.label || stop.locationLabelSnapshot; return <div className="sheet-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="stop-detail-sheet" role="dialog" aria-modal="true" aria-label="Stop details"><button className="sheet-close" onClick={onClose}>×</button><div className="detail-type"><span>{stop.movementType === "collection" ? "COLLECTION" : "DELIVERY"}</span><b>{statusLabel(stop.status)}</b></div><h2>{stopLabel(stop, data)}</h2><p className="detail-address">⌖ {address}</p><div className="detail-time"><span>Scheduled window</span><strong>{timeFor(stop)}</strong></div><div className="detail-block"><h3>Load summary</h3><p>{movement?.items.map((item) => `${item.quantity} ${item.unit || "items"} · ${item.description}`).join(" · ") || itemSummary(stop) || "Operational load details attached"}</p></div>{stop.notes && <div className="detail-note">✎ {stop.notes}</div>}{(stop.issues || []).filter((issue) => issue.status === "open").map((issue) => <div className="detail-issue" key={issue.id}>⚠ {issue.description}</div>)}<div className="detail-actions"><button className="navigate-action" onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address)}`, "_blank")}>Navigate</button>{stop.status === "planned" && <button onClick={() => onAction("arrive-stop", stop)}>Mark arrived</button>}{stop.status === "arrived" && <button onClick={() => onAction("complete-stop", stop)}>Complete stop</button>}<button className="issue-action" onClick={onIssue}>Report issue</button></div></section></div>; }
function IssueSheet({ stop, type, setType, text, setText, onClose, onSubmit }: { stop: DeliveryStop; type: string; setType: (value: string) => void; text: string; setText: (value: string) => void; onClose: () => void; onSubmit: () => void }) { return <div className="sheet-backdrop" role="presentation"><section className="stop-detail-sheet issue-sheet" role="dialog" aria-modal="true" aria-label="Report an issue"><button className="sheet-close" onClick={onClose}>×</button><p className="driver-section-kicker">REPORT EXCEPTION</p><h2>What needs attention?</h2><p className="issue-stop-name">{stop.locationLabelSnapshot}</p><label>Issue type<select value={type} onChange={(event) => setType(event.target.value)}>{["Unable to complete", "No access", "Site closed", "Delayed", "Customer unavailable", "Partial completion", "Collection issue", "Load issue", "Other"].map((item) => <option key={item}>{item}</option>)}</select></label><label>Notes<textarea value={text} onChange={(event) => setText(event.target.value)} placeholder="Add a short note for the planner…" rows={4} /></label><button className="submit-issue" disabled={!text.trim()} onClick={onSubmit}>Submit issue</button></section></div>; }
...
function timeFor(stop: DeliveryStop) { const window = stop.plannedWindow || stop.window; return window ? `${window.startTime}${window.endTime ? ` – ${window.endTime}` : ""}` : stop.plannedArrivalTime || stop.requiredTime || "Time to confirm"; }
function mobileStopMinutes(stop: DeliveryStop) { const value = stop.plannedWindow?.startTime || stop.window?.startTime || stop.plannedArrivalTime || stop.requiredTime; if (!value) return Number.MAX_SAFE_INTEGER; const [hours, minutes] = value.split(":").map(Number); return hours * 60 + minutes; }
