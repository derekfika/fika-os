...
export default function MobileWorkflow({ fixedVan }: { fixedVan?: "Van 1" | "Van 2" } = {}) {
  ...
  const load = async () => withDataTrace({ app: "logistics", action: fixedVan ? "logistics.mobile.van.load" : "logistics.mobile.day.load", path: typeof window === "undefined" ? "/mobile" : window.location.pathname }, async () => { try { const response = await fetch(`/api/logistics?serviceDate=${date}${fixedVan ? `&vehicle=${encodeURIComponent(fixedVan.toLowerCase().replace(" ", ""))}` : ""}`, { cache: "no-store" }); const body = await response.json().catch(() => null); if (!response.ok) throw new LogisticsResponseError(responseErrorDetails(body, response.status, "Logistics is temporarily unavailable.")); const fallback = body?.projection ? projectionToDashboardData(body.projection) : undefined; const useFallback = fallback && (!body.runs?.length || !body.stops?.length); setData(useFallback ? { ...fallback, requirements: body.requirements || fallback.requirements, movements: body.movements || fallback.movements, oplocs: body.oplocs || fallback.oplocs, projection: body.projection } : body); setError(""); } catch (cause) { setError(cause instanceof LogisticsResponseError ? `${cause.message}${cause.details.requestId ? ` Reference: ${cause.details.requestId}` : ""}` : cause instanceof Error ? cause.message : "Logistics is temporarily unavailable."); } });
...
function StopDetail({ stop, data, onClose, onAction, onIssue }: { stop: DeliveryStop; data: Data; onClose: () => void; onAction: (action: string, stop: DeliveryStop, extra?: Record<string, unknown>) => void; onIssue: () => void }) {
  ...
  return <div className="sheet-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="stop-detail-sheet" role="dialog" aria-modal="true" aria-label="Stop details"><button className="sheet-close" onClick={onClose}>×</button><div className="detail-type"><span>{collection ? "COLLECTION" : "DELIVERY"}</span></div><h2>{stopLabel(stop, data)}</h2><p className="detail-address">⌖ {address}</p><div className="detail-time"><span>{collection ? "Collection time" : "Delivery time"}</span><strong>{timeFor(stop)}</strong></div><div className="detail-block"><h3>{collection ? "Collection items" : "Load checklist"}</h3><p className="detail-help">Tap each subload to progress it from loaded to delivered.</p>{subloads.length ? <ul className="detail-subloads">{subloads.map((subload) => { const delivered = stop.status === "completed" || (subload.kind === "requirement" && Boolean(stop.deliveredRequirementIds?.includes(subload.id))); const loaded = stop.loaded === true || (subload.kind === "requirement" && Boolean(stop.loadedRequirementIds?.includes(subload.id))); const label = delivered ? "Delivered" : loaded ? "Tap to mark delivered" : "Tap to mark loaded"; return <li key={subload.id}><button type="button" className={`subload-card ${delivered ? "is-delivered" : loaded ? "is-loaded" : ""}`} aria-pressed={delivered} onClick={() => toggleSubload(subload)}><span className="subload-check" aria-hidden="true">{delivered ? "✓" : loaded ? "•" : ""}</span><span><b>{subload.category}</b><small>{subload.description}</small><em>{label}</em></span><strong>{subload.quantity} {subload.unit}</strong></button></li>; })}</ul> : <p>Operational load details attached</p>}</div>{stop.notes && <div className="detail-note">✎ {stop.notes}</div>}{(stop.issues || []).filter((issue) => issue.status === "open").map((issue) => <div className="detail-issue" key={issue.id}>⚠ {issue.description}</div>)}<div className="detail-actions"><button className="navigate-action" onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address)}`, "_blank")}>Navigate</button>{completed ? <button onClick={() => onAction("undo-completion", stop)}>{collection ? "Undo collection" : "Undo completion"}</button> : <button onClick={() => onAction("complete-stop", stop, { confirmDirect: true })}>{collection ? "Mark received" : "Mark delivered"}</button>}<button className="issue-action" onClick={onIssue}>Report issue</button>{collection && !completed && <PostponeCollectionControl onPostpone={(targetServiceDate) => onAction("defer-collection", stop, { targetServiceDate })} />}</div></section></div>;
}
