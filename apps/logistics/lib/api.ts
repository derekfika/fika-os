...
function readableErrorMessage(error: unknown) {
  ...
  if (error instanceof z.ZodError) {
    ...
    if (paths.includes("expectedVersion")) return "This Logistics record has changed since you opened it. Refresh and try again.";
    ...
    return "Some Logistics information was incomplete. Refresh and try again.";
  }
  ...
  if (status === 404) return "We could not find that Logistics record. Refresh and try again.";
  if (status === 409) return "Logistics changed elsewhere. Refresh before trying again.";
