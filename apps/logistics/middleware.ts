...
export async function middleware(request: NextRequest) {
  ...
  if (failure.status === 401) { const target = new URL(process.env.NEXT_PUBLIC_FIKA_HUB_URL || hub); target.searchParams.set("returnTo", request.nextUrl.href); target.searchParams.set("launchError", failure.code); target.searchParams.set("appId", "logistics"); return NextResponse.redirect(target); }
