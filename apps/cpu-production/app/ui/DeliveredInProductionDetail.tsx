...
export default function DeliveredInProductionDetail({ order, orders, close, onSaved }: { order: ProductionOrder; orders: ProductionOrder[]; close: () => void; onSaved: () => Promise<void> }) {
  ...
  useEffect(() => { let cancelled = false; const loadReview = async () => { const orderIds = [...new Set(orders.map(item => item.canonicalId))]; if (!orderIds.length) { setCpuReviewed(false); return; } const response = await fetch(`/api/production-plan?reviewStatus=1&orderIds=${encodeURIComponent(orderIds.join(","))}`, { cache: "no-store" }); if (!response.ok || cancelled) { if (!cancelled) setCpuReviewed(false); return; } const body = await response.json() as { reviewStatuses?: Array<{ orderId: string; reviewed: boolean }> }; const statuses = new Map((body.reviewStatuses || []).map(status => [status.orderId, status])); const reviewed = orders.every(item => statuses.get(item.canonicalId)?.reviewed === true); if (!cancelled) setCpuReviewed(orders.length > 0 && reviewed); }; void loadReview(); return () => { cancelled = true; }; }, [orders]);
  ...
  return <aside className="delivered-in-detail" role="dialog" aria-modal="true" aria-label="Delivered-In production detail">
    <header><div><small>Delivered-In · {order.destinationLabel || order.destinationOplocId || "Destination"}</small><h2>{dateLabel(order)}</h2><p>{totals.portions.toLocaleString()} portions allocated to this destination</p></div><button type="button" onClick={close} aria-label="Close production detail">×</button></header>
