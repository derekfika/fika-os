...
<table class="matrix"><thead><tr><th>Dish / food / product</th>${matrixColumns.map(([, label]) => `<th>${escapeHtml(label)}</th>`).join("")}<th>May contain notes<br>Gluten / tree nuts</th></tr></thead><tbody>${rows || `<tr><th colspan="${matrixColumns.length + 2}">No sub-items recorded.</th></tr>`}</tbody></table>
