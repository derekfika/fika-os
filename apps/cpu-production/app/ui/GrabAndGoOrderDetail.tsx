...
export default function GrabAndGoOrderDetail({ order, close }: { order: ProductionOrder; close: () => void }) {
  ...
  return <aside className="grab-order-detail" role="dialog" aria-modal="true" aria-label="Grab and Go order detail">
    ...
    <section><h3>What was ordered</h3><p className="grab-order-detail-muted">Customer-ordered quantities for the destination below.</p><div className="grab-order-detail-lines">{order.lines.map(line => <div key={line.canonicalId}><strong>{line.customerQuantity.toLocaleString()}</strong><span>{line.itemName}<small>{line.customerUnit}</small></span></div>)}</div></section>
