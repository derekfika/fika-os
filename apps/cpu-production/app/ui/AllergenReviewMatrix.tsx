...
export default function AllergenReviewMatrix({ rows, orders, locked = false, onSaved, onCheckedChange, onReviewChanged, onRegisterSave, onSignatureRolesChange }: { rows: AllergenReviewRow[]; orders: ProductionOrder[]; locked?: boolean; onSaved?: () => Promise<void>; onCheckedChange?: (checked: number, total: number, keys: Set<string>) => void; onReviewChanged?: () => void; onRegisterSave?: (save: () => Promise<void>) => void; onSignatureRolesChange?: (roles: Array<"production_chef" | "head_chef_site_manager">) => void }) {
  ...
  const markChecked = async (rowKey: string) => {
    if (checkedRows.has(rowKey) || busy || locked) return;
