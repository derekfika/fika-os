...
export async function middleware(request: NextRequest) {
  ...
  if (request.nextUrl.pathname === "/auth-error") return NextResponse.next();
  let response: Response | undefined;
  try {
    response = await fetch(`${hubUrl()}/api/cpu-production/access`, {
  ...
  if (request.nextUrl.pathname.startsWith("/api/")) {
    return NextResponse.json(
      { error: { message: response?.status === 401 ? "A FIKA OS session is required." : response?.status === 403 ? "CPU Production access is denied." : "FIKA OS authentication service is unavailable." } },
  ...
  errorUrl.searchParams.set("kind", response?.status === 403 ? "denied" : "unavailable");
  return NextResponse.rewrite(errorUrl, { headers: { "Cache-Control": "no-store" } });
}
