...
test("legacy CPU saved allergen data is readable as canonical data", () => {
  ...
  const source = Object.values(fixture).flatMap(plan => plan.menuItems || []).flatMap(item => item.subItems || []).find(item => item.allergens && ("noKeyAllergens" in item.allergens || "otherNuts" in item.allergens));
  assert.ok(source);
