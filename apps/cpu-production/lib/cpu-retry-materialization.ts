...
import { currentAllergenReleaseMatchesOrder, matrixSignatureScope, signatureAuthorityForOrder, signatureMatchesScope, type MatrixSignatureScope, type ProductionPlan } from "../app/lib/production-plan";
...
function sameLineage(left: MatrixSignatureScope | undefined, right: MatrixSignatureScope) {
  return Boolean(left && left.productionOrderId === right.productionOrderId && left.serviceDate === right.serviceDate && left.sourceDayId === right.sourceDayId && left.sourcePublicationId === right.sourcePublicationId && left.sourcePublicationDayId === right.sourcePublicationDayId && left.sourceVersion === right.sourceVersion && left.sourceContentHash === right.sourceContentHash && left.matrixContentHash === right.matrixContentHash);
}
...
export async function retryCommittedCpuMaterialization(input: {
  ...
  if (!sameLineage(currentScope, expectedLineage)) throw Object.assign(new Error("The reviewed Menu publication has changed. Reload and review the current matrix before retrying materialization."), { status: 409, code: "CPU_RELEASE_LINEAGE_CONFLICT" });
