...
export function publishedAllergenMatrixHtml(day: PublishedMatrixDay) {
  ...
  const rows = day.entries.map(entry => `<tr><th>${escapeHtml(entry.slot)} · ${escapeHtml(entry.dishName)}</th>${CANONICAL_ALLERGEN_COLUMNS.map(([key]) => { const state = entry.allergens[key] || "unrecorded"; return `<td class="${state}">${state === "may_contain" ? "MC" : state === "unrecorded" ? "UR" : ""}</td>`; }).join("")}<td class="notes">${escapeHtml(entry.mayContainNotes || "—")}</td></tr>`).join("");
