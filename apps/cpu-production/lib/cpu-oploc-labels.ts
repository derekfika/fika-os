...
export async function withReadableDestinations(request: NextRequest, orders: ProductionOrder[]) {
  ...
  const labels = new Map((await hubJson(request, "/api/oplocs", { method: "GET", headers: { accept: "application/json" } }, (value): value is { oplocs: Array<{ canonicalId: string; label: string }> } => Boolean(value && typeof value === "object" && Array.isArray((value as { oplocs?: unknown }).oplocs)))).oplocs.map(oploc => [oploc.canonicalId, oploc.label] as const));
