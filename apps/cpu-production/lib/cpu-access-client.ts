...
export async function requireCpuActor(request: NextRequest): Promise<CpuActor> {
  ...
  const base = (process.env.FIKA_HUB_BASE_URL || "http://localhost:3200").replace(/\/$/, ""); let response: Response;
  try { response = await fetch(`${base}/api/cpu-production/access`, { cache: "no-store", signal: AbortSignal.timeout(8_000), headers: request.headers.get("cookie") ? { cookie: request.headers.get("cookie")! } : {} }); } catch { throw Object.assign(new Error("Integration Hub is unavailable."), { status: 503 }); }
