...
1. Inspect the relevant UI, API route, domain/service layer, persistence layer, shared contracts and tests.
2. Identify which domain owns the state.
3. Trace downstream and upstream callers before changing a shared type or contract.
4. Search for nested `AGENTS.md`, local README/docs and package scripts in the affected tree.
5. Prefer the smallest coherent change that preserves platform invariants.
...
2. inspect current domain tests and live call paths;
3. consult project history/invariants;
4. report the conflict rather than silently choosing whichever source is easier.
